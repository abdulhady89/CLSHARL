...
class RULEBASED:
    ...
    def RuleBasedAct(self, agent_id, sats_current_state):
        ...
        obs_names = [self.envs.env.satellites[agent_id].observation_description[k].item(
        ) for k in range(self.envs.env.observation_space[agent_id].shape[0])]
        sats_current_state_list = sats_current_state.tolist()[0]
        detailed_obs = dict(
            zip(obs_names, sats_current_state_list[:len(obs_names)]))
        ...
        # Battery Level Check
        if current_state['sat_props']['battery_charge_fraction'] > 0.8:
            ...
            if current_state['sat_props']['wheel_speeds_fraction[0]'] <= 0.8 and current_state['sat_props']['wheel_speeds_fraction[1]'] <= 0.8 and current_state['sat_props']['wheel_speeds_fraction[2]'] <= 0.8:
                ...
                if current_state['sat_props']['storage_level_fraction'] < 0.9:
                    ...
                    if 'OPT' in sat_name:
                        priorities = [current_state['target'][f'target_{i}']['priority'] for i in range(
                            int((n_acts-n_basic_act)/2))]
                    else:
                        priorities = [current_state['target'][f'target_{i}']['priority'] for i in range(
                            n_acts-n_basic_act)]
        ...
        # Battery Low, Eclipse Check
            ...
            act = 0  # Charging
            print(
                f"{sat_name} Charging at battery = {current_state['sat_props']['battery_charge_fraction']}")
